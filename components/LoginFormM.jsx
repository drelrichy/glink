...
export default function LoginForm({ params }) {
  ...
  const inMessage = searchParams.get('message');
  const inError = searchParams.get('error');
  const inActivate = searchParams.get('activate');

  useEffect(() => {
    const init = async () => {
     // const { Datepicker, Input, initTE, Collapse, Ripple } = await import("tw-elements");
      //initTE({ Datepicker, Input, Collapse, Ripple });


    };

  if (inActivate)  handleActivate(inActivate);
if (inMessage) setMessage(inMessage);
   if (inError) setError(inError);
    init();
  }, []);
  ...
  const handleActivate = async (token) => {
    ...
    if (!token) {
      setError("a valid Token is Required");
      return;
    }
    ...
    try {
      ...
      console.log("fThe Return ------------  -->", feedback);
  ...
  if (password.length < 6) {
    ...
    try {
      const feedback = await signIn("credentials", {
        email,
        password,
        redirect: false,
      });
...
//------------------------------------------
    ...
    //redirect("/dashboard");
      ...
      console.log("feeeeeee---------Master ************------------------d back-->",feedback);
      ...
      if (feedMessage) {
        ...
        const doneIt = "/login?message=" + feedMessage;
        //router.push(doneIt);   
        setError("");
        setMessage(feedMessage);
  ...
  if (runOnce) {
    ...
    //if (inActivate) {  const token = inActivate; handleActivate(inActivate);  }
  ...
  if (session) redirect("/profile");
  const pagePath = usePathname();// window.location.pathname;
 // if (typeof window != 'undefined') {

    //------window.history.replaceState({}, "New Day", pagePath );
        ...
        <div className="h-full">
          ...
          <div
            ...
            <div className="mb-12 md:mb-0 md:w-8/12 lg:w-5/12 xl:w-5/12">
              {message && (
                <div className="bg-blue-500 text-white w-fit text-lg py-1 px-3 rounded-md mt-2">
                  {message}
                </div>
              )}
              {error && (
                <div className="bg-red-500 text-white w-fit text-lg py-1 px-3 rounded-md mt-2">
                  {error}
                </div>
              )}
              <form onSubmit={handleSubmit}  >
                ...
                <div className="text-center lg:text-left">
                  ...
                  <button
                    ...
                    className="inline-block bg-green-900   rounded  px-7 pb-2.5 pt-3 text-sm font-medium uppercase leading-normal text-white shadow-[0_4px_9px_-4px_#3b71ca] transition duration-150 ease-in-out hover:bg-primary-600 hover:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.3),0_4px_18px_0_rgba(59,113,202,0.2)] focus:bg-primary-600 focus:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.3),0_4px_18px_0_rgba(59,113,202,0.2)] focus:outline-none focus:ring-0 active:bg-primary-700 active:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.3),0_4px_18px_0_rgba(59,113,202,0.2)] dark:shadow-[0_4px_9px_-4px_rgba(59,113,202,0.5)] dark:hover:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.2),0_4px_18px_0_rgba(59,113,202,0.1)] dark:focus:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.2),0_4px_18px_0_rgba(59,113,202,0.1)] dark:active:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.2),0_4px_18px_0_rgba(59,113,202,0.1)]"
                    data-te-ripple-init
